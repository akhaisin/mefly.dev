...
This package lives in the [mefly.dev](../../) monorepo and is consumed by the
site through the pnpm workspace — there is no separate publish step. Reference it
from any workspace package via:
...
Then run `pnpm install` at the repo root. The site's root `build` script builds
this package (`pnpm --filter mefly-nav build`) before Astro runs, so its `dist/`
is always up to date in CI and fresh clones.

For live editing during development, run the package's watcher alongside the site:

```bash
pnpm --filter mefly-nav dev   # vite build --watch
pnpm dev                      # astro dev (separate terminal)
```
